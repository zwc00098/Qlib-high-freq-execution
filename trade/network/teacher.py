...
class Teacher_1_Extractor(nn.Module):
...
class Teacher_2_Extractor(nn.Module):
    #increase the depth#
    def __init__(self, device="cpu", feature_size=180, **kargs):
        super().__init__()
        self.device = device
        ...
        self.rnn = nn.LSTM(64, hidden_size, 2, batch_first=True)
        self.rnn2 = nn.GRU(64, hidden_size, 2, batch_first=True)
        self.dnn = nn.Sequential(nn.Linear(2, 64), nn.ReLU(),)
...
class Teacher_3_Extractor(nn.Module):
    #Simply increase the width by increasing the dim of feature#
    def __init__(self, device="cpu", feature_size=180, **kargs):
        super().__init__()
        self.device = device
        hidden_size = kargs["hidden_size"]
        ...
        self.rnn2 = nn.GRU(64, hidden_size, batch_first=True)
        self.dnn = nn.Sequential(nn.Linear(2, 64), nn.ReLU(),)
        self.cnn = nn.Sequential(nn.Conv1d(self.cnn_shape[1], 6, 3), nn.ReLU(),)
        self.raw_fc = nn.Sequential(nn.Linear((self.cnn_shape[0] - 2) * 6, 64), nn.ReLU(),)
...
class Teacher_1_Actor(nn.Module):
...
class Teacher_2_Actor(nn.Module):
    def __init__(self, extractor, out_shape, device=torch.device("cpu"), **kargs):
        super().__init__()
        self.extractor = extractor
        self.layer_out = nn.Sequential(nn.Linear(32, out_shape), nn.Softmax(dim=-1))
...
class Teacher_3_Actor(nn.Module):
    def __init__(self, extractor, out_shape, device=torch.device("cpu"), **kargs):
        super().__init__()
        self.extractor = extractor
        self.layer_out = nn.Sequential(nn.Linear(32, out_shape), nn.Softmax(dim=-1))
...
class Teacher_1_Critic(nn.Module):
    def __init__(self, extractor, out_shape, device=torch.device("cpu"), **kargs):
        super().__init__()
        self.extractor = extractor
...
class Teacher_2_Critic(nn.Module):
    def __init__(self, extractor, out_shape, device=torch.device("cpu"), **kargs):
        super().__init__()
        self.extractor = extractor
        self.value_out = nn.Linear(32, 1)
...
class Teacher_3_Critic(nn.Module):
    def __init__(self, extractor, out_shape, device=torch.device("cpu"), **kargs):
        super().__init__()
        self.extractor = extractor
        self.value_out = nn.Linear(32, 1)
