...
class PPO_sup_L1(PGPolicy):
    """The PPO policy with a MSE supervision loss"""
    ...
    def learn(self, batch: Batch, batch_size: int, repeat: int, **kwargs) -> Dict[str, List[float]]:
        ...
        for _ in range(repeat):
            for b in batch.split(batch_size):
                ...
                supervision_loss = MSE_loss(b.act, b.teacher_action)
                supervision_losses.append(supervision_loss.item())
                kl = torch.distributions.kl.kl_divergence(self.dist_fn(b.old_logits), dist)
                kl_loss = kl.mean()
                kl_losses.append(kl_loss.item())
...
class PPO_sup_L2(PGPolicy):
    """The PPO policy with a combination of MSE and log-likelihood supervision losses"""
    ...
    def learn(self, batch: Batch, batch_size: int, repeat: int, **kwargs) -> Dict[str, List[float]]:
        ...
        for _ in range(repeat):
            for b in batch.split(batch_size):
                ...
                b.teacher_action = torch.clamp(b.teacher_action, 0, 6)
                MSE_loss = torch.nn.MSELoss()
                supervision_loss = 0.5 * MSE_loss(b.act, b.teacher_action) + 0.5 * F.nll_loss(logits.log(), b.teacher_action)
                supervision_losses.append(supervision_loss.item())
                kl = torch.distributions.kl.kl_divergence(self.dist_fn(b.old_logits), dist)
                kl_loss = kl.mean()
                kl_losses.append(kl_loss.item())
        ...
        for _ in range(repeat):
            for b in batch.split(batch_size):
                ...
                if self._value_clip:
                    ...
                    vf_loss = (b.returns - value).pow(2).mean()
                b.teacher_action = torch.clamp(b.teacher_action, 0, 6)
                supervision_loss = F.smooth_l1_loss(b.act, b.teacher_action)
                supervision_losses.append(supervision_loss.item())
                kl = torch.distributions.kl.kl_divergence(self.dist_fn(b.old_logits), dist)
                kl_loss = kl.mean()
                kl_losses.append(kl_loss.item())
