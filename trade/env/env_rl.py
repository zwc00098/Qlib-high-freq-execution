...
class StockEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for i in range(self.time_interval):
            ...
            if self.limit >= 1:
                max_vol_t = np.inf
            if v_t > min(self.position + self.t0_position * self.target, max_vol_t):
                if self.position + self.t0_position * self.target <= max_vol_t:
                    v_t = self.position + self.t0_position * self.target
                else:
                    v_t = max_vol_t
            if v_t < max(self.position - (1 + self.t0_position) * self.target, - max_vol_t):
                if self.position - (1 + self.t0_position) * self.target <= - max_vol_t:
                    v_t = - max_vol_t
                else:
                    v_t = self.position - (1 + self.t0_position) * self.target
            if v_t < 0 and self.t0_traded - v_t > self.t0_position * target:
                v_t = self.t0_traded - self.t0_position * target
            if v_t > 0:
                v_t = min(v_t, (1 + self.t0_position) * self.target - self.sell)
            if v_t < 0:
                v_t = max(v_t, self.buy - self.t0_position * self.target)
            self.position -= v_t
        ...
        if self.buy >= self.t0_position * self.target and self.sell >= (1 + self.t0_position) * self.target:
            self.done = True
        ...
        if self.done:
            this_vwap = ((self.money_sell - self.money_buy) / (self.sell - self.buy)) if self.sell > ZERO else self.day_vwap
...
class StockEnv_Acc(StockEnv):
    def step(self, action):
        ...
        v_t = np.maximum(v_t, - max_vol_t)

        for i in range(len(v_t)):
            if v_t[i] > 0 and v_t[i] * time_left > (1 + self.t0_position) * self.target - self.sell:
                v_t[i] = ((1 + self.t0_position) * self.target - self.sell) / time_left
            if v_t[i] < 0 and v_t[i] * time_left < self.buy -  self.t0_position * self.target:
                v_t[i] = (self.buy -  self.t0_position * self.target) / time_left
        ...
        this_vol = v_t.sum()
        this_vwap = np.nan_to_num(this_money / this_vol) if np.abs(this_vol) > ZERO else self.day_vwap
        self.t += time_left
        self.position -= this_vol
        if this_vol > 0:
            self.sell += this_vol
            self.money_sell += this_money
        if this_vol < 0:
            self.buy -= this_vol
            self.money_buy -= this_money
        if self.log:
        ...
        if self.done:
            this_vwap = ((self.money_sell - self.money_buy) / (self.sell - self.buy)) if self.sell > ZERO else self.day_vwap
            valid = min(self.target, self.this_valid)
            this_ffr = ((self.sell - self.buy) / valid) if valid > ZERO else 1.0
            if abs(this_ffr - 1.0) < ZERO:
                this_ffr = 1.0
            this_ffr *= 100
            this_vv_ratio = this_vwap / self.day_vwap
